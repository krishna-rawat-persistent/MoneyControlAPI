...
public class StockFilterService {
    public List<StockFilter> getStocks(){
        ...
        final String url = "https://www.moneycontrol.com/mutual-funds/performance-tracker/returns/focused-fund.html";
        try {
            ...
            for(Element row: document.select("table.mctable1 tr")) {
                ...
                String col1Y = row.select("td:nth-of-type(10)").text();
                String col2Y = row.select("td:nth-of-type(11)").text();
                String col3Y = row.select("td:nth-of-type(12)").text();
                String col5Y = row.select("td:nth-of-type(13)").text();
                String col10Y = row.select("td:nth-of-type(14)").text();
        ...
        return LimitRecords.getTopNRecords(SortingLogic.sortedList(stocks,"tenYear"),5);
    }
}
